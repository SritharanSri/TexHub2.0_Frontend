...
import {
  ...
  Gavel, User, Settings, LogOut, X, Scissors
} from 'lucide-react'
...
const navGroups = [
  ...
  },
  {
    label: 'Orders',
    items: [
      { to: '/orders/new',       icon: ShoppingBag,  label: 'New Orders' },
      { to: '/orders/bidding',   icon: Gavel,        label: 'Bidding' },
      { to: '/orders/process',   icon: Clock,        label: 'On Process' },
      { to: '/orders/completed', icon: CheckCircle,  label: 'Completed' },
    ],
  },
  {
    label: 'Account',
      ...
      <div className="px-4 py-4 mx-3 mt-4 rounded-2xl bg-white/5 border border-white/10">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="text-gray-500 text-xs truncate capitalize">{user?.role || 'Tailor'}</p>
          </div>
